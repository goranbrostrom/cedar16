...
\begin{frame}[fragile]{Infant mortality by branch, Skellefteå 1801--1950}
...
oj.nofa <- infmort(obs[obs$region == "ske" & obs$socBranch != "farming", ],
                   1801:1950)
plot(names(oj.nofa), oj.nofa, type = "l", col = "red", ylim = c(0, 25),
     axes = FALSE, xlab = "Year", ylab = "IMR (per cent)")
axis(1, at = c(1801, 1851, 1901, 1950))
axis(2, las = 1)
box()
lines(names(oj.farm), oj.farm, col = "blue")
abline(h = 0)
@ 
\end{frame}
