...
\item Skellefteå and Umeå 1901--1950.
  ...
  \item Description
  \item Research
      and influence of \emp{SES}.
    \begin{enumerate}
    \item \emp{Fertility}
    \item \emp{Adult mortality}
     \end{enumerate}
    \end{itemize}
\end{itemize}
...
\item Uttag \emp{U15014}, Version 2 (4 February 2016): \emp{Poplink}.
  ...
  \end{itemize}
\item {\bf \emp{R}} package \emp{\tt skum} 
  \begin{itemize}
    \item PERSON: Two data frames:
      \begin{itemize}
        \item \emp{\tt persons}
        \item \emp{\tt per}  
      \end{itemize}
...
\begin{frame}[fragile]{Infant mortality by branch}
...
plot(spline(1801:1950, fems), type = "l", ylim = c(0, 6.3), xlab = "Year", ylab = "Births per woman", axes = FALSE, col = "red")
...
\begin{frame}[fragile]{e0 by sex, 1901-1950}
...
oldpar <- par(mfrow = c(1, 2))
plot(spline(yrs[1:50], smm), type = "l", ylim = c(45, 75), axes = FALSE, col = "blue", xlab = "Year", ylab = "e0", main = "Skellefteå")
axis(1, at = c(1901, 1918, 1950), las = 1)
axis(2, las = 1)
box()
lines(spline(yrs[1:50], sfm), type = "l", col = "red")
abline(v = c(1901, 1918, 1950),  lty = 3, col = "darkgreen")
text(1925, 67, "Women", col = "red")
text(1925, 50, "Men", col = "blue")
...
lines(spline(yrs[1:50], ufm), lty = 1, type = "l", col = "red")
text(1925, 67, "Women", col = "red")
text(1925, 50, "Men", col = "blue")
abline(v = c(1901, 1918, 1950),  lty = 3, col = "darkgreen")
abline(h = c(60, 70), lty = 3, col = "darkgreen")
...
fit <- phreg(Surv(enter - 15, exit - 15, birth) ~ strata(region), data = ww,
             dist = "pch", cuts = 1:33)
plot(fit, fn = "haz", col = c("red", "blue"), xlab = "Age - 15")
@   
\end{frame}
...
<<fralcum>>=
plot(fit, fn = "cum", col = c("red", "blue"), 
     xlab = "Age - 15", ylim = c(0, 4.5))
abline(h = 2.1, lty = 3, col = "darkgreen")
abline(h = 0)
@ 
\end{frame}

\begin{frame}[fragile]{Cumulative hazards by socBranch}
  
<<fralcumsoc>>=
ww$socBranch <- relevel(ww$socBranch, ref = "farming")
fit <- phreg(Surv(enter - 15, exit - 15, birth) ~ strata(region) + socBranch, 
data = ww, dist = "pch", cuts = 1:33)
plot(fit, fn = "cum", col = c("red", "blue"), 
     xlab = "Age - 15")
abline(h = 2.1, lty = 3, col = "darkgreen")
abline(h = 0)
