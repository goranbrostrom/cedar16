    ...
    4, 3), ...) 
{
    ...
    etxt <- "Risk Ratio"
    cat(paste("Covariate & Mean & Coef &", etxt, " & S.E. & ", 
        ptxt, " \\\\ \\hline\n"))
    ...
    lpindx <- 0
    if (FALSE) {
        cat("(Intercept)", " & ", " ", " & ", coef[index], " & ", 
            e.coef[index], " & ", se[index], " & ")
        if (lp) {
            lpindx <- lpindx + 1
            ppv <- lpval[lpindx]
        }
    ...
    #     index <- index + 1
    #     cat(formatC(ss.names[i], width = 16, flag = "-"), formatC(" & & ", 
    #         width = 8, digits = 3, format = "c"), coef[index], 
    #         " & ", e.coef[index], " & ", se[index], " & ", formatC(wald.p[index], 
    #             digits = 3, width = digits + 2, format = "f"), 
    #         " \\\\ ", "\n")
    # }
    # cat("\\hline \n")
